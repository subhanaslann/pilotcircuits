...
attributes to camelCase, lit bindings dropped, <defs> ids prefixed per part so
two parts sharing one document cannot capture each other's gradients.
...
def to_jsx(el, exprs, prefix, indent=2):
    ...
    for k, v in el.attrib.items():
        ...
        jk = kebab_to_camel(k)
        if k == 'id':
            attrs.append('%s="%s-%s"' % (jk, prefix, v)); continue
        if k == 'style':
            attrs.append('style=%s' % css_to_obj(v, exprs)); continue
        v2 = re.sub(r'url\(#([^)]+)\)',
                    lambda m: 'url(#%s-%s)' % (prefix, m.group(1)), v)
...
def fix_uno(body, pins_female):
    ...
    def glow(m):
        return ('{%s ? (\n          <circle cx="%s" cy="%s" r="%s" fill="%s" '
                'filter="url(#uno-ledFilter)" />\n        ) : null}' %
                (m.group('flag'), m.group('cx'), m.group('cy'),
                 m.group('r'), m.group('fill')))
    body = re.sub(
...
def generate(src_dir, dest):
    ...
    for s in specs:
        ...
        out = HEADER + s.get('imports', '') + TEMPLATE.format(
