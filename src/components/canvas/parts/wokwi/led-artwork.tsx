 ...
 *
 * Source: https://github.com/wokwi/wokwi-elements (MIT).
 *
 * Hand-carried rather than generated: upstream the LED is a `<div>` wrapping an
 * `<svg>` plus a label span, and the parts of it we want — the dome and the
 * glow — are inside `renderSVG()`. The label is dropped here because the canvas
...
/** 5 mm through-hole LED — -10 -5 35.456 39.618, rendered 40 x 50 CSS pixels. */
export function LedArtwork({
  color = "red",
  lightColor,
  value = false,
  brightness = 1,
  ...
  uid: string;
  /**
   * Applied to the glow group alone.
   *
   * The one hook this port has for animating the light without touching the
   * body: the dome is eight semi-transparent layers, so anything that
   * composites a second copy over it moves the tint as well as the
   * brightness. Chapter one's breath rides on this.
   */
  glowClass?: string;
}) {
  ...
  return (
    <g>
      ...
      {/* the two internal posts, seen through the dome */}
      <polygon
      ...
      {/* the tint — this is the only place the body colour is applied */}
      <path
        d="m14.173 13.001v-5.9126c0-3.9132-3.168-7.0884-7.0855-7.0884-3.9125 0-7.0877 3.1694-7.0877 7.0884v13.649c1.4738 1.651 4.0968 2.7526 7.0877 2.7526 4.6195 0 8.3686-2.6179 8.3686-5.8594v-1.5235c-7.4e-4 -1.1426-0.47444-2.2039-1.283-3.1061z"
        fill={color}
        opacity=".65"
