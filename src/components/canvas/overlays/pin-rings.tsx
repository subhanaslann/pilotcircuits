...
export function WrongPinMark({ pin }: { pin: CircuitNode }) {
  return (
...
export function TargetPinMark({ pin }: { pin: CircuitNode }) {
  const r = PITCH * 0.82;
  return (
    <g className="motion-safe:motion-pop">
      <circle
...
export function CorrectionCallout({
  ...
  const midX = (wrong.x + target.x) / 2;
  /* Sit above the header, clear of the pin labels. */
  const boxY = Math.min(wrong.y, target.y) - PITCH * 7.2;
  ...
  return (
    <g className="motion-safe:motion-pop">
      {/* Leader lines. */}
      {[
        { pin: wrong, tone: "var(--color-wire-error)" },
        { pin: target, tone: "var(--color-wire-target)" },
      ].map(({ pin, tone }) => (
        <path
          key={pin.id}
